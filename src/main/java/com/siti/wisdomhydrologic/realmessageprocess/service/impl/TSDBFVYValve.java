 ...
 * @data ${DATA}-9:54
 */
public class TSDBFVYValve implements Valve<TSDBVo, FVYEntity, AbnormalDetailEntity>, ApplicationContextAware {
    ...
    public void beforeProcess(List<TSDBVo> realList) {
        ...
        Map<Integer, FVYEntity> ws = Optional.of(abnormalDetailMapper.fetchAllFVY())
                .get()
                .stream()
                .collect(Collectors.toMap(FVYEntity::getSensorCode, b -> b));
        Map<Integer, TSDBVo> map = realList.stream()
                .filter(
                        e -> ((e.getSENID() + "").substring(5)).equals(ConstantConfig.WFV)
                ).collect(Collectors.toMap(TSDBVo::getSENID, a -> a));
        doProcess(map,ws);
    }

    @Override
    public void doProcess(Map<Integer, TSDBVo> mapval, Map<Integer, FVYEntity> configMap) {
                    ...
                    IntStream.range(0, 13 - limit).forEach(j -> {
                        ...
                        if (flag[0] == limit) {
                            exceptionContainer[0].add(new AbnormalDetailEntity.builer()
                                    ...
                                    .dateError(DataError.WS_INTER_Y.getErrorCode())
                                    .build());
                        }
                    });
                }else{
                        ...
                        if (timelimit[0] > config.getDuration() / 5) {
                            exceptionContainer[0].add(new AbnormalDetailEntity.builer()
                                    .date(LocalDateUtil
                                            ...
                                            .format(DateTimeFormatter.ofPattern("yyyy-MM-dd HH:mm:ss")))
                                    .sensorCode(vo.getSENID())
                                    .dateError(DataError.WS_DURA_Y.getErrorCode())
                                    .build());
                        }
                    } else {
                        doubles[0] = arrayV[k];
