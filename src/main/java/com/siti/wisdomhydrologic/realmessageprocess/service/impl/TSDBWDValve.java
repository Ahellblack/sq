 ...
 * @data ${DATA}-9:54
 */
public class TSDBWDValve implements Valve<TSDBVo, WDEntity, AbnormalDetailEntity>, ApplicationContextAware {
    ...
    public void beforeProcess(List<TSDBVo> realList) {
        ...
        //获取风速度配置表
        Map<Integer, WDEntity> ws = Optional.of(abnormalDetailMapper.fetchWD())
                .get()
                .stream()
    ...
    @Override
    public void doProcess(Map<Integer, TSDBVo> mapval, Map<Integer, WDEntity> configMap) {
                    ...
                    IntStream.range(0, 13 - limit).forEach(j -> {
                        ...
                        if (flag[0] == limit) {
                            exceptionContainer[0].add(new AbnormalDetailEntity.builer()
                                    ...
                                    .dateError(DataError.WS_INTER_WD.getErrorCode())
                                    .build());
                        }
                    });
                }else{
                    logger.error("tsdbwd中断次数不能超过12次");
                }
                        ...
                        if (timelimit[0] > config.getDuration() / 5) {
                            exceptionContainer[0].add(new AbnormalDetailEntity.builer()
                                    .date(LocalDateUtil
                                            ...
                                            .format(DateTimeFormatter.ofPattern("yyyy-MM-dd HH:mm:ss")))
                                    .sensorCode(vo.getSENID())
                                    .dateError(DataError.WS_DURA_WD.getErrorCode())
                                    .build());
                        }
                    } else {
                        doubles[0] = arrayV[k];
