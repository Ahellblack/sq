...
public class UploadController {
    ...
    private static final String URL = "C:/Users/dell/Desktop/zyw/";
    //设置文件最大限制
    ...
    public JSONObject krryupload(@RequestParam(value = "files", required = false) MultipartFile file, HttpServletRequest request) throws IllegalStateException, IOException, JSONException {
        ...
        //调用工具类完成上传，返回相关数据到页面
        return uploadFile(file, MAX_FILE_SIZE,ipAddress,user);
    }
    ...
    public  JSONObject uploadFile(MultipartFile file, long maxPicSize,String ipAddress,User user) throws IOException {
       ...
       /* //获得文件的扩展名称
        ...
        File uploadFilePath = new File(URL);
...
*/
        ...
        //按照时间戳修改文件名称
        String localUrl = URL+timestamp + "_" + file.getOriginalFilename();//防止重复上传文件出错，添加时间戳后缀
        ...
        try {
            fileUploadMapper.saveFileMsg(localUrl, user.getRealName(), ipAddress);
        }catch (Exception e){
