...
public class StationBrokenServiceImpl implements StationBrokenService {
    ...
    private static final Logger logger = LoggerFactory.getLogger(StationBrokenServiceImpl.class);

    private static final int STATUS = 2;

    public PageInfo<com.siti.wisdomhydrologic.operation.entity.ReportStationBroken> getAll(HttpSession session, int page, int pageSize, String createDate, String stationId, Integer status) {
        ...
        List<com.siti.wisdomhydrologic.operation.entity.ReportStationBroken> all = reportStationBrokenMapper.getAll(createDate, stationId, orgList.get(0).getId(), status, 1);
    ...
    //查询全部数据
    public PageInfo<com.siti.wisdomhydrologic.operation.entity.ReportStationBroken> selectAllDisplay(int page, int pageSize, String createDate, String stationId, Integer status) {
        User user = (User) userInfoService.get();
        List<Org> orgList = userMapper.findOrg(user.getId());
        //默认查询本月
        if (createDate == null) {
        ...
        PageHelper.startPage(page, pageSize);
        List<com.siti.wisdomhydrologic.operation.entity.ReportStationBroken> all = reportStationBrokenMapper.getAll(createDate, stationId, orgList.get(0).getId(), status, null);

        return new PageInfo<>(all);
    }

    @Override
    public int insert(com.siti.wisdomhydrologic.operation.entity.ReportStationBroken entity) {
        try {
            ...
            reportStationBrokenMapper.insert(entity);
        } catch (Exception e) {
            return 0;
        }
        return 1;
    ...
    @Override
    public int update(com.siti.wisdomhydrologic.operation.entity.ReportStationBroken entity) {

        try {
            entity.setErrorLastestAppearTime(entity.getCreateTime());
            reportStationBrokenMapper.update(entity);
        } catch (Exception e) {
            return 0;
        }
        return 1;
    ...
    public int updateMalStatus(Integer reportId) {
        try {
            com.siti.wisdomhydrologic.operation.entity.ReportStationBroken entity = reportStationBrokenMapper.getOne(reportId);
            ConfigRiverStation malStation = configRiverStationMapper.getAllByCode(entity.getStationId());
            List<String> phoneNumber = reportStationBrokenMapper.getNumberByRegionId(malStation.getRegionId());
            /**
            ...
            PushMsg.pushMsgToClient(
                    ...
                    reportId + "");
            return reportStationBrokenMapper.updateStatus(entity);

        } catch (Exception e) {
            System.out.println("派单异常"+e);
            return 0;
        }
    }

    public int updateBrokenStatus() {
        List<com.siti.wisdomhydrologic.operation.entity.ReportStationBroken> notResolveList = reportStationBrokenMapper.getNotResolve();
        ...
        try {
            notResolveList.forEach(data -> {
                ...
                if (unCoverData == 0) {
                    ...
                    reportStationBrokenMapper.updateStatus(data);
                }
            });
        } catch (Exception e) {
    ...
    public Map<String, Object> updateModuleStatus(Integer reportId) {
        ...
        try {
             ...
             * 对审核的设备故障，替换成已完成状态
             * */
            Optional<com.siti.wisdomhydrologic.operation.entity.ReportStationBroken> optional = Optional.ofNullable(reportStationBrokenMapper.getOne(reportId));

            com.siti.wisdomhydrologic.operation.entity.ReportStationBroken data = optional.get();
            String today = DateTransform.Date2String(new Date(), "yyyy-MM-dd HH:mm:ss");
            data.setRequestDesignatingStatus(4);
            data.setBrokenResolveTime(today);
            if (data.getRequestDesignatingTime() == null) {
             ...
             * */
            Integer sensorCode = reportStationBrokenMapper.findModule(reportId);
    ...
    public int delete(Integer reportId) {
        return reportStationBrokenMapper.updateDisplayStatus(reportId);
    }
                ...
                if (histroyList.size() > 0) {
                    histroyList.forEach(data -> lastest.forEach(lasttime -> {
                        if (data.getReportId() == lasttime.getId()) {
                            reportStationBrokenMapper.updateTime(data.getReportId(), lasttime.getLastDate());
    ...
    public Workbook exportSheetByTemplate(HttpSession session, String createTime, String stationId, Integer status) {
        ...
        // 查询数据,此处省略
        List<com.siti.wisdomhydrologic.operation.entity.ReportStationBroken> list = reportStationBrokenMapper.getAll(createTime, stationId, orgList.get(0).getId(), status, 1);

        for (int i = 0; i < list.size(); i++) {
            com.siti.wisdomhydrologic.operation.entity.ReportStationBroken data = list.get(i);
        ...
        // 设置导出配置
        // 获取导出excel指定模版
        TemplateExportParams params = new TemplateExportParams( "sqexcelmodel/model4.xls");
        // 标题开始行
    ...
    public Workbook exportSheetByTemplate(HttpSession session, String createTime, String stationId, @RequestParam List<Integer> reportIdList, Integer status) {
        ...
        if (reportIdList.size() > 0) {
            List<com.siti.wisdomhydrologic.operation.entity.ReportStationBroken> reportlist = new ArrayList<>();
            for (int i = 0; i < list.size(); i++) {
