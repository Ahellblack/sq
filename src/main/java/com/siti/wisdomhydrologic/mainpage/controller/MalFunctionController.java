...
public class MalFunctionController {
    ...
    @ApiOperation(value = "首页北片的派单状况", httpMethod = "GET",
            notes = "北片数据派单状况,默认展示年派单数据," +
                    " NstationNumber北测站数" + " NmalNumber北派单数" + " NonResolveNumber北维护中数"
                    + " NendResolveNumber北已解决数" + " NnormalStationNumber北正常测站数" + " NabnormalStationNumber北异常测站数"
                    + " NdownStationNumber北离线测站数")
    @ApiParam(name = "Date", value = "date值若不传则默认查询年派单数据" +
            "date = 1时 查询日数据 date = 2时 查询月数据 date = 3 查询年数据")
    @GetMapping("/getN")
    ...
    @ApiOperation(value = "首页南片的派单状况", httpMethod = "GET",
            notes = "南片数据派单状况,默认展示年派单数据," +
                    " NstationNumber北测站数" + " NmalNumber南派单数" + " NonResolveNumber南维护中数"
                    + " NendResolveNumber南已解决数" + " NnormalStationNumber南正常测站数" + " NabnormalStationNumber南异常测站数"
                    + " NdownStationNumber南离线测站数")
    @ApiParam(name = "Date", value = "date值若不传则默认查询年派单数据" +
            "date = 1时 查询日数据 date = 2时 查询月数据 date = 3 查询年数据")
    @GetMapping("/getS")
    ...
    @ApiOperation(value = "首页南片的畅通率状况", httpMethod = "GET",
            notes = "南片数据派单状况,默认展示年派单数据，每个测站畅通率及畅通率不合格数," +
                    "返回值为map key 'UnPatencyNumber' 为不畅通数量" +
                    "key 'List' 为不畅通测站信息及畅通率")
    @GetMapping("/Spatency")
    ...
    @ApiOperation(value = "首页北片的畅通率状况", httpMethod = "GET",
            notes = "北片数据派单状况,默认展示年派单数据，每个测站畅通率及畅通率不合格数," +
                    "返回值为map key 'UnPatencyNumber' 为不畅通数量" +
                    "key 'List' 为不畅通测站信息及畅通率")
    @GetMapping("/Npatency")
